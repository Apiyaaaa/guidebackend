...
def getArticle(article_id=None, title=None,page = 1):
    ...
    if not article_id and not title:
        results = Article.query.paginate(page= int(page), per_page=20, error_out=False).items
        if len(results) == 0:
            msg = f"数据库无文章"
        ...
        results = Article.query.filter_by(article_id=article_id).paginate(page=int(page), per_page=20, error_out=False).items
        if len(results) == 0:
...
def uploadimg(imgdata):
    ...
    url = 'http://localhost:8080/static/upload/img/' + finalName
