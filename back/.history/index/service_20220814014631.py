...
def getArticle(word, page):
    if word == '':
        results = Article.query.filter_by(is_publish='1').paginate(page=page, per_page=20, error_out=False).item()
    else:
        results = Article.query.filter(Article.title.like(
            "%" + str(word) + "%")).filter_by(is_publish='1').paginate(page=page, per_page=20, error_out=False).item()
