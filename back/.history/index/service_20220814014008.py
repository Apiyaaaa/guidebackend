...
def getArticle(word, page):
    index = page*20
    if word == '':
        results = Article.query.filter_by(is_publish='1').paginate(page=page, perpage=20, error_out=False)
    else:
        results = Article.query.filter(Article.title.like(
            "%" +  + "%")).filter_by(is_publish='1').paginate(page=page, perpage=20, error_out=False)
    results = query2dict(results)
    data = data[index-20:index]
